...
**Goal:** Select appropriate model (cogito:8b for simple, cogito:14b for balanced, gpt-oss:20b for complex)
...
| `model` | No | inherit | `cogito:8b`, `cogito:14b`, `gpt-oss:20b`, or omit to inherit |
...
|---------------|-------|------|----------|
| `cogito:8b`   | Fast | Low | Simple tasks, quick lookups, formatting |
| `cogito:14b`  | Balanced | Medium | Most tasks, research, analysis, writing |
| `gpt-oss:20b` | Slower | High | Complex reasoning, strategy, difficult problems |
| (omit)        | Inherit | Inherit | Use whatever the main session uses |

**Rule of thumb:** Start with `cogito:14b`, only use `gpt-oss:20b` if you need it.
